...
public class SoundTouch {
	...
	private boolean testFloatEqual(float a, float b){
	...
	public void setChannels(int numChannels){
		if (numChannels != 1 && numChannels != 2) {
			throw new Exception("Illegal number of channels");
		}
	...
	public void calcEffectiveRateAndTempo(){
		...
		if (!testFloatEqual(rate,oldRate)) pRateTransposer.setRate(rate);
		if (!testFloatEqual(tempo, oldTempo)) pTDStretch.setTempo(tempo);
	...
	void putSamples(byte[] samples, int nSamples){
		if (bSrateSet == false) throw new Exception("SoundTouch : Sample rate not defined");
		else if (channels == 0) throw new Exception("SoundTouch : Number of channels not defined");
		else if (rate <= 1.0f) {
	...
	public void flush()
	{
		...
		int nUnprocessed;
		int nOut;
		byte[] buff = new byte[64*2];   // note: allocate 2*64 to cater 64 sample frames of stereo sound
	...
	public boolean setSetting(int settingId, int value){
		...
		pTDStretch.getParameters(&sampleRate, &sequenceMs, &seekWindowMs, &overlapMs); //TODO:
